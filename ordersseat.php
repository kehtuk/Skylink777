...
// Обработка добавления нового заказа места
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_order_seat'])) {
    $order_id = $_POST['order_id'];
    $seat_id = $_POST['seat_id'];

    // Вставляем новый заказ места в базу данных
    $stmt = $conn->prepare("INSERT INTO OrderSeats (order_id, seat_id) VALUES (?, ?)");
    $stmt->bind_param("ii", $order_id, $seat_id);

    if ($stmt->execute()) {
        echo "Заказ места успешно добавлен.";
    } else {
...
// Получаем список заказов мест
$result = $conn->query("SELECT order_seat_id, order_id, seat_id FROM OrderSeats");
?>
...
<h1>Управление заказами мест</h1>

<form method="POST">
    <h3>Добавить заказ места</h3>
    <label for="order_id">Order ID:</label>
    <input type="number" name="order_id" required><br>
    <label for="seat_id">Seat ID:</label>
    <input type="number" name="seat_id" required><br>
    <button type="submit" name="add_order_seat">Добавить заказ</button>
</form>

<h3>Список заказов мест</h3>
<table>
    <tr>
        <th>Order Seat ID</th>
        <th>Order ID</th>
    ...
    <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
                <td><?php echo $row['order_seat_id']; ?></td>
                <td><?php echo htmlspecialchars($row['order_id']); ?></td>
                <td><?php echo htmlspecialchars($row['seat_id']); ?></td>
                <td>
                    <a href="edit_ordersseat.php?id=<?php echo $row['order_seat_id']; ?>">Изменить</a> |
                    <a href="delete_ordersseat.php?id=<?php echo $row['order_seat_id']; ?>">Удалить</a>
                </td>
            </tr>
        <?php endwhile; ?>
    <?php else: ?>
